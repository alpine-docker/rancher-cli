...
# Fork from https://github.com/etlweather/gaucho/blob/master/services.py
...
# HOST = "https://rahcner.example.com/v2-beta"
...
def post(url, data=""):
    if data:
        r = requests.post(
            url, data=json.dumps(data), auth=(USERNAME, PASSWORD), **kwargs)
    else:
...
@baker.command(
    default=True,
    params={"service_id": "The ID of the service to read (optional)"})
...
#
# Converts a stack name into an ID
#
@baker.command(params={"stack_name": "The stack name"})
def stack_id(stack_name=""):
    """Retrieves the stack information.
   """

    service = get(HOST + "/stacks?name=" + stack_name + "&&accountId=" +
                  accountId).json()
...
def service_id(name="", newest=False):
   ...
   """
    ...
    service = get(HOST + "/services?name=" + service_name + "&&accountId=" +
                  accountId + "&&stackId=" + stackId).json()
    return service['data'][0]['id']
   ...
   """
    ...
    if name == "":
        if ENVIRONMENT != "":
            ...
            print "Need environment variable 'RANCHER_ENVIRONMENT' or params"
            print "Usage: rancher.py env_id dev"
            quit()
...
#
@baker.command(
    params={"service_id": "The ID of the service to start the containers of."})
...
def start_service(service_id):
   ...
   """
    ...
    containers = get(HOST + URL_SERVICE + service_id +
                     "/instances").json()['data']
    for container in containers:
        start_url = container['actions']['start']
        print "Starting container %s with url %s" % (container['name'],
                                                     start_url)
        post(start_url, "")
...
#
@baker.command(
    params={"service_id": "The ID of the service to stop the containers of."})
def stop_service(service_id):
   ...
   """
    ...
    for container in containers:
        ...
        print "Stopping container %s with url %s" % (container['name'],
                                                     stop_url)
        post(stop_url, "")
...
@baker.command(params={
    "service_id":
    "The ID of the service to restart the containers of."
})
...
@baker.command(
    params={
        ...
        "start_first":
        "Whether or not to start the new instance first before stopping the old one.",
        "complete_previous":
        "If set and the service was previously upgraded but the upgrade wasn't completed, it will be first marked as Finished and then the upgrade will occur.",
        "imageUuid":
        "If set the config will be overwritten to use new image. Don't forget Rancher Formatting 'docker:<Imagename>:tag'",
        "auto_complete":
        "Set this to automatically 'finish upgrade' once upgrade is complete",
        "replace_env_name":
        "The name of an environment variable to be changed in the launch config (requires replace_env_value).",
        "replace_env_value":
        "The value of the environment variable to be replaced (requires replace_env_name).",
        "timeout":
        "How many seconds to wait until an upgrade fails"
    })
def upgrade(service_id,
            ...
            replace_env_name=None,
            replace_env_value=None,
            timeout=60):
    """Upgrades a service
   ...
   """
    ...
    upgrade_strategy['inServiceStrategy'][
        'launchConfig'] = current_service_config['launchConfig']
    ...
    if replace_env_name != None and replace_env_value != None:
        print "Replacing environment variable %s from %s to %s" % (
            replace_env_name, upgrade_strategy['inServiceStrategy']
            ['launchConfig']['environment'][replace_env_name],
            replace_env_value)
        upgrade_strategy['inServiceStrategy']['launchConfig']['environment'][
            replace_env_name] = replace_env_value

    if imageUuid != None:
        # place new image into config
        upgrade_strategy['inServiceStrategy']['launchConfig'][
            'imageUuid'] = imageUuid
...
def execute(service_id, command):
  ...
  """
    ...
    payload = json.loads(
        '{"attachStdin": true,"attachStdout": true,"command": ["/bin/sh","-c"],"tty": true}'
    )
...
@baker.command(
    params={
        "service_id": "The ID of the service to deactivate.",
        "timeout": "How many seconds to wait until an upgrade fails"
    })
...
@baker.command(
    params={
        "environment_id": "The ID of the environment to deactivate.",
        "timeout": "How many seconds to wait until an upgrade fails"
    })
...
#
@baker.command(
    default=True, params={"service_id": "The ID of the service to read"})
...
if __name__ == '__main__':
    ...
    # This python cli need four system environment variables:
    # 'RANCHER_ACCESS_KEY',' RANCHER_SECRET_KEY', 'RANCHER_URL', 'RANCHER_ENVIRONMENT'
    if ('RANCHER_ACCESS_KEY' not in os.environ) or (
            'RANCHER_SECRET_KEY' not in os.environ) or (
                'RANCHER_URL' not in os.environ) or (
                    'RANCHER_ENVIRONMENT' not in os.environ):
        print "This python cli need four system environment variables:"
        print "'RANCHER_ACCESS_KEY',' RANCHER_SECRET_KEY', 'RANCHER_URL', 'RANCHER_ENVIRONMENT'"
        quit()
    else:
        USERNAME = os.environ['RANCHER_ACCESS_KEY']
        PASSWORD = os.environ['RANCHER_SECRET_KEY']
        HOST = os.environ['RANCHER_URL']
        ENVIRONMENT = os.environ['RANCHER_ENVIRONMENT']
